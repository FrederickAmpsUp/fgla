...
namespace std {
template <>
struct hash<fgla::Queue::Request> {
...
std::pair<std::vector<VkDeviceQueueCreateInfo>, QueueAllocator::QueueMapping> QueueAllocator::big_brain_allocator_algorithm(const std::initializer_list<std::reference_wrapper<const Queue::Request>>& requests, std::vector<VkQueueFamilyProperties> families) {
	...
	std::unordered_map<Queue::Type, std::pair<uint32_t, std::vector<FamilyInfo *>>> counts_and_available_families;
		...
		for (int i = 0; i < count; ++i) {
			...
			if (best_score >= 0 && best_family) {
				final_mapping.insert({
					{type, i},
					{best_family->index, best_family->used_queues.size()}
				});
				best_family->used_queues.push_back(type);
			} else if (best_family) {
				final_mapping.insert({
					{type, i},
					{best_family->index, idx_fallback_queue}
				});	
			} else {
