...
void DeviceImpl::init_shader(
  ...
  slang::createGlobalSession(&global_session_desc,
                             this->slang_global_session.writeRef());
  ...
  this->slang_global_session->createSession(session_desc,
                                            this->slang_session.writeRef());
}

Result<ShaderModule>
DeviceImpl::load_shader_module(const ShaderModule::Descriptor &desc) {
  Slang::ComPtr<slang::IBlob> diagnostics;
  auto module = Slang::ComPtr<slang::IModule>(this->slang_session->loadModule(
      desc.module_name.c_str(), diagnostics.writeRef()));
  ...
  SlangResult link_result =
      module->link(linked_program.writeRef(), diagnostics.writeRef());
  ...
  SlangResult code_result = linked_program->getTargetCode(
      0, spirv_code.writeRef(), diagnostics.writeRef());
  ...
  VkResult shader_module_result = vkCreateShaderModule(
      this->device, &shader_module_info, nullptr, &shader_module);
