...
DeviceImpl::DeviceImpl(AdapterImpl& adapter, const Device::Descriptor& descriptor, QueueAllocator& queue_allocator) {
	static auto logger = spdlog::get("fgla::backends::vulkan");

	std::span<VkDeviceQueueCreateInfo> queue_create_infos = queue_allocator.get_queue_create_infos();
	...
	VkResult result = vkCreateDevice(adapter.get_physical_device(), &create_info, nullptr, &this->device);
...
bool DeviceImpl::is_ok() const {
	return this->device != VK_NULL_HANDLE;
}
