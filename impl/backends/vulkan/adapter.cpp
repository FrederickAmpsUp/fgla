...
AdapterImpl::AdapterImpl(const InstanceImpl &instance_impl, const Adapter::Descriptor &descriptor) {
  ...
  for (VkPhysicalDevice candidate : devices) {
    ...
    logger->info(" - \"{}\", vendor \"{}\", score {}", candidate_props.deviceName, vendor_name,
                 candidate_score);
...
bool AdapterImpl::is_ok() const { return this->physical_device != VK_NULL_HANDLE; }
...
Adapter::Info AdapterImpl::get_info() const {
  if (!this->is_ok()) {
    return Adapter::Info{.device_name = "none"};
  }
  Adapter::Info info;
