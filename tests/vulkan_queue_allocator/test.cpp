...
void print_requests(const std::initializer_list<fgla::Queue::Request> &requests) {
  ...
  for (auto &req : requests) {
    std::cout << "	[" << i++ << "] Type: " << static_cast<int>(req.type)
              << ", Count: " << req.count << '\n';
  }
}
...
// Print results
void print_results(const std::vector<VkDeviceQueueCreateInfo> &createInfos,
...
void test_big_brain_allocator() {
  ...
  }
  {
    ...
    std::vector<VkQueueFamilyProperties> fams{
        {.queueFlags = VK_QUEUE_GRAPHICS_BIT, .queueCount = 3},
