...
int main(int argc, char **argv) {
  ...
  std::cout << 
    "== Ready to log window events. Move your mouse or press some keys.\n";
  ...
  while (window.is_open()) {
    ...
    for (const auto& event : events)
    {
      ...
      if (std::holds_alternative<KeyEvent>(event)) {
        const auto& ev = std::get<KeyEvent>(event);
        std::cout << "== KeyEvent\n   pressed: [";
        for (size_t i = 0; i < ev.pressed.size(); i++)
        {
          if (i > 0) {
            std::cout << ", ";
          }
          std::cout << KEY_CODE_NAMES.at(ev.pressed[i]);
        }
        std::cout << "]\n   released: [";
        for (size_t i = 0; i < ev.released.size(); i++)
        {
          if (i > 0) {
            std::cout << ", ";
          }
          std::cout << KEY_CODE_NAMES.at(ev.released[i]);
        }
        std::cout << "]\n";
      }
      else if (std::holds_alternative<MouseEvent>(event)) {
        const auto& ev = std::get<MouseEvent>(event);
        printf(
          "== MouseEvent\n   old:   %6.1f, %6.1f\n   new:   %6.1f, %6.1f\n"
          "   delta: %6.1f, %6.1f\n",
          ev.old_x, ev.old_y, ev.new_x, ev.new_y, ev.delta_x, ev.delta_y
        );
      }
      else {
        std::cout << "== unknown Event variant\n";
      }
    }
  }
