...
constexpr BackendUUID parse_backend_uuid(const char *str) {
    ...
    if (byte_index != 16)
        throw "UUID must be exactly 16 bytes";
...
struct BackendRegistrar {
	BackendRegistrar(const Backend& backend) {
...
inline bool is_available(BackendUUID uuid) {
	...
	if (it == registry.end() || it->second.is_available == nullptr || it->second.create_instance == nullptr) return false; // backend doesn't exist
